...
@eel.expose
def search_ichiba_item(keyword):
    """
     商品名から楽天商品検索
    """
    print("call:{}".format(sys._getframe().f_code.co_name))
    result_disp = ""
    try:
        result_disp = control.get_ichiba_item(keyword)
        
    except HttpStatusRakutenException as hsre:
        write_error_log(hsre)
        result = {"result":"ng", "message":"エラー情報：{}".format(hsre)}
        return result
...
def search_product(keyword):
    ...
    result_disp = ""
    try:
        result_disp = control.get_product(keyword)
...
def search_genre(keyword):
    ...
    result_disp = ""
    try:
        result_disp = control.get_genre(keyword)
...
def search_rank(keyword):
    ...
    result_disp = ""
    try:
        result_disp = control.get_rank(keyword)
